...
public class DataUsuarios {
	...
	public void add(Usuario newUser) {
		...
		try {
			...
			stmt = DbConnector.getInstancia().getConn().prepareStatement("INSERT INTO usuarios "
					+ "(username, password, nombre, apellido, email, telefono, idRol)"
					+ " values (?,?,?,?,?,?,?)",
			...
			stmt.setInt(7, newUser.getRol().getIdRol());
			stmt.executeUpdate();
	...
	public Usuario getByUsername(String username) {
		...
		ResultSet rs = null;
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement("select idUsuario, username, nombre, apellido, email, telefono from usuarios where username = ?");
		...
		try {
			...
			if(rs!=null) {
				...
				while(rs.next()) {
					...
					item.setFecha_de_reserva(rs.getObject("fecha_de_reserva", LocalDate.class));
					item.setFecha_reservada(rs.getObject("fecha_a_reservar", LocalDate.class));
					...
					item.setIdpc(rs.getInt("idComputadora"));
					list.add(item);
				}
			return list;
			}
